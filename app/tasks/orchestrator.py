...
def _run_agent(analysis_id: str, agent_name: str, lat: float, lon: float, **kwargs) -> dict:
    ...
    try:
        ...
        if agent_name == "airspace":
            ...
            # Placeholder for other agents (Days 3-5)
            result = {
                "score": 50,
                "summary": f"{agent_name.capitalize()} agent not yet implemented.",
                "warnings": [f"{agent_name} analysis pending implementation"],
                "raw_data": {"status": "not_implemented", "agent": agent_name},
            }
...
def run_analysis_pipeline(analysis_id: str):
    ...
    if lat is None or lon is None:
        raise ValueError(f"Analysis {analysis_id} missing lat/lon — geocoding must run first")
